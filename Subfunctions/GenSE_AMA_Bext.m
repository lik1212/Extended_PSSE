function [x_hat, z_hat, z_hat_full, Out_Optional] = GenSE_AMA_Bext(z_all_data, z_all_flag, LineInfo, Inputs_SE)
%GenSE_AMA_Bext General State Estimation with augmented matrix approach & 
...
H_flag_new = [H_flag_new; C_flag(ismember(H_index(C_flag,:).Meas_Type, [3, 4]))]; % Bext for C_flag
...
z_sigma = [...
    ...
    1 + (z_all_flag_order.Sigma(z_all_flag_order.Accur_Type == 3 & ismember(z_all_flag_order.Meas_Type, [3, 4]))).^2]; % Bext, this part is still not right. How to add the virtuall measurements twice, once "exakt" and once with Bext.
R         = diag(z_sigma); % Covariance matrix 
alpha     = min(diag(R));
...
for k_inst = 1 : num_inst
    ...
    z_input_new = [...
        z_all_data_order(1 : HC_switch - 1 , k_inst); ...
        z_all_data_order(ismember(z_all_flag_order.Meas_Type(1 : HC_switch - 1), [3, 4]), k_inst); ...
        z_all_data_order(HC_switch : end , k_inst); ...
        z_all_data_order([false(HC_switch - 1, 1); ismember(z_all_flag_order.Meas_Type(HC_switch : end), [3, 4])], k_inst); ...
        ];  % Extended measurements
    delta_x = NaN(num_Nodes * 6, 1); % Initial
    for k_iter = 1 : max_iter % iteration
        ...
        z_k_hat = z_SE([H_flag_new; C_flag]);                     % Estimated measurement vector z_hat
        delta_z = z_input_new - z_k_hat;
        % sum(delta_z.^2)  % Temp
        if all(abs(delta_z) < z_conv) || all(abs(delta_x) < x_conv) % Convergence limit reached
        ...
        HC_switch_new = size(H_flag_new,1) + 1;
        H_SE = get_H_SE(Y_L1L2L3, Y_012_Node_ID, x_k_hat); % Get the H matrix of SE
        H_AM = H_SE(H_flag_new,:);                             % Matrix of real-pseudo values, AM stands for augmented matrix
        H_AM(numel(H_flag) + 1 : end, :) = H_AM(numel(H_flag) + 1 : end, :) / 2;
        H_AM(numel(H_flag) + 1 : end, num_Nodes * 3 + 1 : end) = 0;
        C_AM = H_SE(C_flag,:);                             % Matrix of virtual     values
        C_AM(numel(C_flag) + 1 : end, :) = C_AM(numel(C_flag) + 1 : end, :) / 2 ;
        C_AM(numel(C_flag) + 1 : end, num_Nodes * 3 + 1 : end) = 0;
        ...
        rhs_AM                  = [      ...      % Create right hand side of the LSE for the augmented matrix approach
            delta_z(1 : HC_switch_new - 1)  ;...
            zeros(size(C_AM,2),1)       ;...
            delta_z(HC_switch_new :   end)  ;...
            ];
        solution_vector = Hachtel \ rhs_AM;
        delta_x(:,1)    = solution_vector(HC_switch_new : end - numel(C_flag)); % State vector correction
    ...
    z_temp = z_SE([H_flag; C_flag]);
    z_hat     (:, k_inst) = z_temp(z_order_back);  % Estimated measurement vector
